...
namespace AWSIM
{
    ...
    public class PredictedPerceptionResultSensor : MonoBehaviour
    {
        /// <summary>
        ...
        void CreateDetectedObjectData()
        {
            ...
            for (int i = 0; i < cachedObjectsWithClassification.Length; i++)
            {
                ...
                if (!objectIdMap.ContainsKey(obj.gameObject))
                {
                    objectIdMap[obj.gameObject] = nextId++;
                }
                outputData.objects[i].id = objectIdMap[obj.gameObject];
